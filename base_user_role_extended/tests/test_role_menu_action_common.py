...
class TestRolePolicyMenuActionCommon(TransactionCase):
    ...
    Branches covered in create():
      - groups_id present + role_policy_init absent → filtered
        • filtered commands non-empty
        • filtered commands empty
      - role_ids present → role groups appended
    ...
    Branches covered in write():
      - groups_id present + role_policy_init absent → filtered
        ...
        • filtered commands empty
      - role_policy_init context → groups_id NOT filtered
      - role_ids present after write → role groups synced
    """
    ...
    def test_write_keeps_untouchable_group(self):
        """write(): a (4, keep_id) command survives the filter."""
    ...
    def test_write_role_groups_also_preserved_by_filter(self):
        ...
        write time, so they survive a non-role write.
        """
        menu = self.env["ir.ui.menu"].create(
            {
    ...
    def test_write_with_role_policy_init_skips_filter(self):
        ...
        """
        custom_group = self.env["res.groups"].create({"name": "Write Init Group"})
    ...
    def test_write_with_role_ids_syncs_group(self):
        ...
        menu.with_context(role_policy_init=True).write({"role_ids": [(4, role2.id)]})
    ...
    def test_write_with_role_ids_no_group_cmds_does_not_write(self):
        ...
        menu.with_context(role_policy_init=True).write({"role_ids": [(5,)]})
